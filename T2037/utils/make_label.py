    ...
    def labeling(self):
        ...
        for idx in tqdm.tqdm(range(train_csv.shape[0])):
            IMG_PATH = os.path.join(
                self.img_path, train_csv.iloc[idx]["path"]
            )  # image path
            ...
            for file in file_list:
                ...
                self.df.loc[idx_df] = train_csv.loc[idx][
                    ["gender", "age"]
        ...
        self.df.to_csv("train_with_label_58.csv")  # csv 파일 형식으로 저장
...
if __name__ == "__main__":
    ...
    BASE_TRAIN_PATH = "../data/train"  # train data path
    train_csv = pd.read_csv(os.path.join(BASE_TRAIN_PATH, "train.csv"))
    # print(train_csv.shape) # (2700, 5)

    make_label = Make_Label(
        BASE_TRAIN_PATH, train_csv, ["gender", "age", "path", "name", "label"]
    )
    make_label.labeling()
